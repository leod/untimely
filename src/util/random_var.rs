...
impl SlidingWindowRandomVar {
    ...
    pub fn record(&mut self, current_time: LocalTime, value: f64) {
        ...
        while self.entries.front().map_or(false, |(oldest_time, _)| {
            current_time >= *oldest_time + self.max_age
        }) {
        ...
        assert!(self
            .entries
            .back()
            .map_or(true, |(newest_time, _)| current_time >= *newest_time));
    ...
    pub fn to_plot_points(&self, current_time: LocalTime) -> Vec<(f64, f64)> {
        self.entries
            .iter()
            .map(|(time, value)| ((*time - current_time).to_secs(), *value))
            .collect()
    }
}
